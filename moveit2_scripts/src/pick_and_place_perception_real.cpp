...
// program variables
static const rclcpp::Logger LOGGER = rclcpp::get_logger("move_group_node_real");
...
public:
  PickAndPlaceTrajectory(rclcpp::Node::SharedPtr base_node_)
    ...
    object_pose_node =
        rclcpp::Node::make_shared("object_pose_node_real", node_options);
    ...
    move_group_node_ =
        rclcpp::Node::make_shared("move_group_node_real", node_options);
    ...
    joint_model_group_robot_ =
        move_group_robot_->getCurrentState(10.0)->getJointModelGroup(
            PLANNING_GROUP_ROBOT);
    joint_model_group_gripper_ =
        move_group_gripper_->getCurrentState(10.0)->getJointModelGroup(
...
private:
  ...
  using DetectedObjects = object_detection::msg::DetectedObjects;
  ...
  void approach_object() {
    ...
    setup_waypoints_target(+0.000, +0.000, -0.112);
    // plan and execute the trajectory
  ...
  void retreat() {
    ...
    // setup the cartesian target
    setup_waypoints_target(+0.000, +0.000, +0.110);
    // plan and execute the trajectory
    plan_trajectory_cartesian();
    execute_trajectory_cartesian();
  }
  ...
  void topic_callback(const DetectedObjects::SharedPtr msg) {
    ...
    if (msg != nullptr && msg->thickness > 0.03) {
